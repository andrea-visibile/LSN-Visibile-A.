...
int main(int argc, char* argv[]){
	...
	for(int d=0; d<depth; d++){ //evoluzione per d=400 generazioni
		...
		if(d%N_migr==0 and d!=0){Transmission();}
		Crossover(); //fa un crossover e inizia una nuova generazione
		

	}
	MPI_Finalize();
	Print(); //stampa i migliori cammini
...
void Transmission(){
	...
	cout << "Migration!!! Best path is migrating to another continent!" << endl;
	MPI_Status stat[4];
	int itag[4]={1,2,3,4};
	...
	MPI_Comm_rank(MPI_COMM_WORLD, &rank);
	MPI_Request req;
	int send[4], receive[4]; //uso vector per mescolare i vettori con random_shuffle
	for(int i=0; i<4; i++){
			send[i]=i;
			
			}  
	for(int i=0; i<4; i++){
			receive[i]=(send[i]+1)%4;
	...
	for(int i=0; i<4; i++){
		...
		if(rank==send[i]){
			cout << rank<< " is sending to "  <<receive[i] << " with tag: " << send[i]<< endl;;
			MPI_Isend(&x[argmin][0],n,MPI_DOUBLE,receive[i],send[i],MPI_COMM_WORLD,&req);
			MPI_Wait(&req,&stat[i]);	
			}
		if(rank==receive[i]){
		
			cout << rank << "is receiving from "<<send[i]<< " with tag: " << send[i] << endl;
			MPI_Recv(&x[argmin][0],n,MPI_DOUBLE,send[i],send[i],MPI_COMM_WORLD,&stat[i]);	
			}
		}
	...
	for(int j=0; j<n; j++){
		x[argsend][j]=x[argmin][j];
		y[argsend][j]=y[argmin][j];
		}
...
void bestl2(){
	...
	ofstream out;
	out.open("l2.dat",ios::app); //stampa
	out << minl2 << endl;
	out.close();
	sort(a,a+pop); //ordino il vettore

	double sum=0;
	out.open("l2_besthalf.dat"+to_string(seed_mpi),ios::app);
...
void clean(){
	ofstream out;
	out.open("l2.dat"+to_string(seed_mpi));
		out <<"";
	out.close();
	out.open("l2_besthalf.dat"+to_string(seed_mpi));
		out <<"";
	out.close();
...
vector <int> vector; //uso vector per mescolare i vettori con random_shuffle
	...
	for (int j=1; j<pop; j++){ //creo una popolazione di vettori mescolati rispetto a quelli iniziali, senza mischiare le x con le y, quindi mescolo degli indici non direttamente x e y
		shuffle(vector.begin(),vector.end(),std::default_random_engine(seed_mpi));
...
void Print(){	
	...
	cout << "Best:  " << best<<endl;
	cout << seed <<": best evaluation: " << Evaluation(best) << endl; 
	ofstream out;
	out.open("mpi.bests"+to_string(seed_mpi));
	for(int i=0; i < n; i++){
